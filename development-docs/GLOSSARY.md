...
| **project passport** (`PASSPORT.md`) | added 2026-08-24 (step 2, owner's decision). The fifth address, and the only one answering **what this project IS**: the product and how its success is measured, the external services and each one's role, what exists today and what does not, the owner's decisions verbatim, what is verified against what was taken on somebody's word, and the open questions. The other four addresses describe the WORK — planned, current, finished, explained — and none of them describes the project. 🔒 It carries the **starting law**: development does not begin while the "minimum sufficient set" still has an unknown row. 🔒 It **evolves in the same pass** as the decision that changes it, never "later". Every claim in it is marked ✅ verified / 📄 from a document / 🗣 on whose word / ⛔ unknown — an unmarked claim is the failure this file exists to prevent |
| **migration map** (`reports/migration-…`) | the first artefact of a move: every piece of the foreign project answered twice — *what it is* and *what it becomes here* — with "no answer" written where the answer belongs to the owner. Not a file list: a file list does not say what to do. Category `migration-` in the same flat `reports/` folder |
