...
# Прогон выбранных гейтов вместо всего prebuild

> Anti-pattern · stable

Перед доставкой были прогнаны `check:types`, `check:i18n` и ещё девять гейтов — руками, списком.
Сборка на сервере упала на десятом, который в список не попал: `check:typography` не пускает строчный
тег заголовка с классами, а в карте markdown их стояло три. Цена — полный цикл сборки на боевой
машине ради ошибки, которую локальная команда находит за четыре секунды.

**Список гейтов ведётся не тобой.** Он записан в `prebuild` (`package.json`) и меняется без твоего
ведома; любой набранный по памяти набор устаревает молча.

**Вместо этого:** `npm run prebuild` целиком — ровно то, что выполнит сервер.

```
npm run prebuild        # весь конвейер, а не девять любимых гейтов
```

---

# Гейты порождённых файлов ложно падают на Windows

> Anti-pattern · stable

`check:config-schemas` и `check:blocks-map` сверяют порождённые файлы с исходниками ПОБАЙТОВО. Git на
Windows выдаёт рабочую копию с CRLF, генератор пишет LF — и оба гейта объявляют восемь схем и каталог
секций «устаревшими». На сервере (Linux, LF) те же гейты зелёные.

**Ловушка в лечении, а не в диагнозе.** Гейт советует `npm run build:config-schemas`; послушавшись, вы
получаете восемь-девять изменённых файлов, где отличается только перевод строки, и коммитите шум,
выглядящий как настоящая правка. Внутри JSON это ещё и не видно: `\r\n` спрятан в строковых значениях.

**Как отличить за одну команду** — сравнить содержимое без учёта переводов строк:

```
diff -q <(tr -d '\r' < было.json) <(tr -d '\r' < стало.json) && echo "только CRLF — откатить"
```

Содержимое совпало — `git checkout --` на эти файлы и продолжать; арбитр здесь сервер, а не Windows.


🔒 **ПОПРАВКА 2026-08-25, куплена в шаге 7.** Запись выше едва не заставила принять НАСТОЯЩИЙ отказ
за ложный. `check:blocks-map` сверяет не только текст карточек, но и **употребление видов по
страницам**; опустошив список блоков главной, я получил честное расхождение — и первым делом решил,
что это опять CRLF.

**И проверка на «только CRLF» была неверной.** Внутри JSON перевод строки закодирован ДВУМЯ
символами — обратной косой и буквой `r`, — а поиск по БАЙТУ возврата каретки удаляет то, чего
в этом месте нет вовсе. Снимать надо экранированную последовательность:

```
diff -q <(sed 's/\r\n/\n/g' было.json) <(sed 's/\r\n/\n/g' стало.json)
```

**Порядок:** сперва пересобрать и посмотреть diff ГЛАЗАМИ; увидел содержательные строки — расхождение
настоящее, чинить. Только когда остаётся один шум переводов строк — снять его и откатить.

🔒 **ВТОРАЯ ПОПРАВКА, 2026-08-25 — куплена сборкой на боевом сервере.** Поправка выше объясняла
механизм верно и всё равно не спасла: скрипт правки был написан строкой для оболочки, и обратные
косые в нём съел heredoc. Счётчик считал экранированные последовательности, а замена била по
настоящим байтам перевода строк — файл выглядел исправленным, гейт был зелёным на Windows и красным
на сервере, где сборка и упала.

**Два вывода, оба практические:**

1. **Правку с обратными косыми пишем ФАЙЛОМ, а не строкой для оболочки.** Тот же урок, что с
   `grep -P` и с длинным текстом: экранирование в heredoc промахивается молча.
2. **Эталон — сервер, и его можно спросить.** `sha256sum` порождённого файла на сервере против
   `sha256sum` у себя: совпало — файл верен для Linux, что бы ни говорил локальный гейт.

🔒 **`sections/SECTIONS.json` теперь ХРАНИТСЯ В ВИДЕ ДЛЯ LINUX и локальный `check:blocks-map` на
Windows КРАСЕН ВСЕГДА** — как и `check:config-schemas`. Это не поломка и не повод «чинить»: на
сервере оба зелёные. Перед коммитом карты блоков прогонять скрипт приведения и сверять сумму с
сервером.

---

## `UNIQUE` с пустой колонкой не защищает от повтора — и молчит об этом

**Симптом.** Синхронизация идемпотентна по отчёту и не идемпотентна по базе. Повторный прогон
рапортует то же число строк, а таблица растёт.

**Причина.** В SQLite `NULL` не равен `NULL`. Составной ключ, в котором одна колонка пуста, уникален
сколько угодно раз: `ON CONFLICT` не находит конфликта и вставляет новую строку.

```sql
UNIQUE (yclients_record_id, service_title)   -- service_title TEXT  (можно NULL)
-- две вставки одной пары с NULL → ДВЕ строки, ключ не сработал
```

**Лечение.** Колонка, входящая в ключ уникальности, объявляется `NOT NULL DEFAULT ''`, а код пишет
пустую строку вместо `NULL`. Пустая строка — обычное значение, и ключ на ней работает.

🔒 **Лечить нужно В ДВУХ местах.** На уже созданной таблице ограничение не появится: `CREATE TABLE IF
NOT EXISTS` ничего не делает, а `LATE_COLUMNS` добавляет колонки, не ограничения. На таких машинах
инвариант держит только код записи — сказать это в схеме рядом с ключом.

✗ Оплачено 2026-08-25, живой филиал: 2123 записи CRM без услуги стали 4246 строками за два прогона.

## Счётчик отправителя не доказывает состояние приёмника

**Симптом.** Отчёт операции говорит «записано 6232», в таблице 8355.

**Причина.** `report.rows++` считает **отправленное**, а не **легшее**. Между ними стоит база со своими
ключами, конфликтами и отказами.

**Лечение.** Доказательство состояния снимается ЗАПРОСОМ К БАЗЕ, а не из отчёта операции. Отчёт — это
показание заинтересованной стороны. Четвёртая ложная проверка в копилку `use-testing`.

## Обратные кавычки в комментариях внутри шаблонной строки рвут её

**Симптом.** Десяток `error TS1005 / TS1443` подряд, и ни один не указывает на причину: компилятор
видит обрыв строки, а не пунктуацию.

**Причина.** `SCHEMA` в `lib/db/index.ts` — template literal. Обратная кавычка в комментарии **внутри**
SQL закрывает строку посреди файла.

**Лечение.** Внутри шаблонной строки имена в комментариях берутся в «ёлочки» или пишутся голыми.
Перед вставкой блока: `(block.match(/`/g) || []).length === 0`.

✗ Оплачено дважды: 2026-08-25 в шаге 10-1 и **повторно в тот же день** в шаге 11-2 — первый урок лежал
в отчёте закрытого подшага, который никто не перечитывает. Поэтому он здесь.

## Диагноз дешевле лечения: красный гейт лечат ПОСЛЕ того, как поняли причину

**Симптом.** Гейт красен, его подсказка советует `npm run build:<что-то>`; послушавшись, получаешь
замусоренный отслеживаемый файл вдобавок к красному гейту.

**Лечение.** Сначала выяснить, ЧЕМ вызван красный: `git status` по затронутым папкам (пусто — значит
не твоё) и сравнение без `\r`. И только потом решать, трогать ли генератор.

✗ Оплачено 2026-08-25: перегенерация карты блоков ради диагноза дала 13 строк CRLF-шума в
`sections/SECTIONS.json`, откачено `git checkout`. Ловушка описана выше в этом же файле — и всё равно
сработала, потому что читалась как «про коммит», а не «про диагностику».

## `WITH … SELECT` в слое данных молча возвращает пусто

**Симптом.** Составной запрос со скалярными подзапросами отдаёт ноль строк. Ни ошибки, ни отказа —
экран рисует аккуратные нули там, где в базе полтора миллиона.

**Причина.** Общее табличное выражение (CTE) в запросе к слою данных не отрабатывает: ответ приходит
без строк, а `.get()` возвращает `undefined`, и весь разбор молча превращается в нули через `?? 0`.

**Лечение.** Тот же смысл выражается **производной таблицей** во `FROM` — она работает.

```sql
-- ✗ пусто и молча
WITH lv AS (SELECT …) SELECT (SELECT COUNT(*) FROM lv) AS n
-- ✅ работает
SELECT COUNT(*) AS n FROM (SELECT …) lv
```

✗ Оплачено 2026-08-25: аналитика показала 0 человек, 0 выручки и 0 приёмов при 1844 людях и 16.4 млн.
Диагноз занял три пробных запроса, потому что отказа не было — а был правдоподобный ответ.

🔒 **Общее правило, ради которого это здесь:** `?? 0` и `COALESCE` превращают «ответа нет» в «ответ
ноль». Там, где ноль — правдоподобное значение, отсутствие ответа обязано отличаться от нуля.

## У `attendance` в YCLIENTS ЧЕТЫРЕ значения, а не два

**Симптом.** Доля неявок 79% при 1326 пришедших и живой клинике.

**Причина.** Раскладка живого филиала: `1` пришёл (1326), `-1` не пришёл (61), `0` ждёт отметки
(3310), `2` подтверждён (1535). Условие «пришёл = 1, всё остальное = не пришёл» записывает в прогульщики
четыре тысячи человек, которые просто записаны или которым администратор не поставил отметку.

**Лечение.** `1` — явка, `-1` — неявка, `0` и `2` — **не решено**. Доля считается только от решённых;
неотмеченное не участвует ни в числителе, ни в знаменателе — это объём небрежности в CRM, а не
поведение людей.

✗ Оплачено 2026-08-25, экран аналитики. После правки доля неявок — 4%.

🔒 **Открытый вопрос, не решённый молча:** 1535 ПРОШЕДШИХ визитов со статусом `2` («подтверждён») не
считаются состоявшимися нигде в продукте — ни в последнем визите, ни в сегментах, ни в правилах.
Решение за владельцем.

---

## Чужой CLI без явного списка ставит ВСЁ

**Симптом.** `npx <tool>@latest --help` не показывает справку, а начинает установку: правит
`package.json`, тянет десяток пакетов и упирается в вопрос «файл уже существует, перезаписать?».

**Причина.** У установщиков в стиле shadcn незнакомый флаг — не «покажи справку», а «делай по
умолчанию», и умолчание чужого пакета никогда не совпадает с составом нашего проекта.

**Лечение.** Разведка — ЧТЕНИЕМ РЕЕСТРА (`curl` по адресу компонента), запуск — только с явным
списком. Ущерб снимается `git checkout package.json package-lock.json` и проверяется
`git diff --ignore-cr-at-eol` по `components/ui`, а не предположением.

✗ Оплачено 2026-08-25: десять лишних пакетов и почти перезаписанные примитивы.

---

## Гейт не отличает код от комментария

**Симптом.** Гейт продолжает падать на файле, где нарушение уже исправлено.

**Причина.** Проверки ищут по ТЕКСТУ файла. Процитируешь снятую разметку в комментарии, объясняя
починку, — гейт найдёт цитату.

**Лечение.** Объяснять словами, не приводя запрещённую конструкцию дословно.

✗ Оплачено 2026-08-25 дважды: `check:typography` и `check:blocks-map`.

---

## Написанное пропадает при неудачной отправке

**Симптом.** Файл не загрузился, человек видит предупреждение — и обнаруживает пустое поле.

**Причина.** `PromptInput` (AI Elements) чистит поле, если обработчик завершился УСПЕШНО: тихий
`return` после `toast.error` для него — успех. `void onSend(…)` усугубляет: промиса нет, и поле
чистится, не дожидаясь результата.

**Лечение.** Обработчик возвращает промис и БРОСАЕТ при любой неудаче. Свой отказ при этом не
объясняется дважды: сообщение о причине не накрывается общим «сервер не ответил».

✗ Оплачено 2026-08-25, экран переписки.
