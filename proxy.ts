...
const PUBLIC_API_PREFIXES = [
  ...
  "/api/telegram/tick",
  // Служба каналов приносит сюда входящие сообщения пациентов: у неё нет сессии,
  // поэтому дверь стережёт общий секрет, а не роль. ✗ Без этой строки гейт закрыл бы
  // её от самой платформы — ровно та ошибка, что уже случилась с tick выше.
  "/api/care/chatpush/webhook",
  // Часовая стрелка: у расписания нет сессии, дверь стережёт общий секрет. ✗ Без этой
  // строки гейт закрыл бы её от самой платформы — ошибка, уже оплаченная на tick телеграма.
  "/api/care/tick",
];
